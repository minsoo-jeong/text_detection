...
gaussian_heatmap_temp = gaussian_heatmap
gaussian_heatmap_aff_temp = gaussian_heatmap_aff

gaussian_heatmap = gaussian_heatmap_temp
gaussian_heatmap_aff = gaussian_heatmap_aff_temp


# initialize once more
def init_variables(cfg):
    global window, sigma, sigma_aff, threshold_point, THRESHOLD_POSITIVE, center, gaussian_heatmap, gaussian_heatmap_aff
    # setting
    window = cfg.window
    sigma = cfg.sigma
    sigma_aff = cfg.sigma_aff
    threshold_point = cfg.threshold_point
    THRESHOLD_POSITIVE = cfg.THRESHOLD_POSITIVE
...
def four_point_transform(image, pts, size):
    ...
    #     [0, image.size()[1] - 1]], dtype="float32")
    dst = np.array([
        [0, 0],
        [image.shape[1] - 1, 0],
...
def add_character(image, bbox, heatmap=gaussian_heatmap):
    ...
    transformed = four_point_transform(heatmap, bbox.astype(np.float32), [height, width])

    image = cv2.max(image, transformed)
...
def add_affinity(image, bbox_1, bbox_2, bbox_1_flag, bbox_2_flag):
    """
    ...
    if (not bbox_1_flag) or (not bbox_2_flag):
...
def generate_target(image_size, character_bbox, weight=None):
    ...
    character_bbox = character_bbox.transpose(2, 1, 0).astype(np.int32)
    height, width, channel = image_size
    target = np.zeros([height, width], dtype=np.uint8)

    src = target
    # validation, if box_area == 0: continue
    valid_flag = (np.max(character_bbox[:, :, 0], axis=1) - np.min(character_bbox[:, :, 0], axis=1) > 0) & \
                 (np.max(character_bbox[:, :, 1], axis=1) - np.min(character_bbox[:, :, 1], axis=1) > 0)

    for bbox in character_bbox[valid_flag]:
        # target = add_character(target, bbox.copy())
        src = add_character(src, bbox.copy(), heatmap=gaussian_heatmap)
...
def generate_affinity(image_size, character_bbox, text, word_count, weight=None):
    """
    ...
    if len(image_size) == 2:
        ...
        all_affinity_bbox = []
        for word, count in zip(text, word_count):
            word = word.strip().replace(" ", "")
            for char_num in range(count - 1):
                try:
                    src, bbox = add_affinity(src, character_bbox[total_letters].copy(),
                                             character_bbox[total_letters + 1].copy(),
                                             valid_flag[total_letters], valid_flag[total_letters + 1])
                except IndexError:
