...
from .evolution import get_sample_point
...
class midlinePredictor(nn.Module):
    ...
    def forward(self, embed_feature, input=None, seg_preds=None, switch="gt"):
        ...
        for i in range(self.iter):
            ...
            midline = contours[i][:, :mid_pt_num] + torch.clamp(self.midline_preds[i](node_feat).permute(0, 2, 1),
                                                                -self.clip_dis, self.clip_dis)[:, :mid_pt_num]
            ...
            new_contour = contours[i] + torch.clamp(self.contour_preds[i](node_feat).permute(0, 2, 1), -self.clip_dis,
                                                    self.clip_dis)[:, :cfg.num_points]
